...
from pyasan.mars_models import RoverName, ROVER_CAMERAS
...
class TestMarsRoverPhotosClient:
    ...
    def test_get_photos_validation(self):
        ...
        with pytest.raises(ValidationError, match="Must specify either sol or earth_date"):
            client.get_photos("curiosity")
        
        # Cannot specify both sol and earth_date
        with pytest.raises(ValidationError, match="Cannot specify both sol and earth_date"):
    ...
    def test_validate_and_format_date_string(self):
        ...
        # Future date
        future_date = (date_type.today().replace(year=date_type.today().year + 1)).strftime("%Y-%m-%d")
